...
class PerturbationAnalyzer:
    ...
    def check_mukhanov_sasaki_regularity(self):
        ...
        # Compute conformal time eta = ∫ dt/a
        dt = np.diff(t)
        a_mid = 0.5 * (a[1:] + a[:-1])
        eta_integral = np.cumsum(dt / a_mid)
        eta = np.zeros_like(t)
        eta[1:] = eta_integral
        
        # Find eta at bounce
        eta_bounce = eta[i_bounce] if i_bounce < len(eta) else eta[len(eta)//2]
        ...
        # Interpolate z(eta) and compute derivatives
        from scipy.interpolate import interp1d
...
def demonstrate_perturbation_analysis():
    """
    Demo function to show perturbation analysis
    """
    print("=" * 70)
    print("PERTURBATION ANALYSIS DEMONSTRATION")
    print("=" * 70)
    
    # First, we need background data - import from bounce
    try:
        from bounce import run_simulation_robust, omega
        print("Running background simulation...")
        bg = run_simulation_robust(phi0=10.0, n_points=50000, t_max=200/omega)
        
        if bg is None:
            print("Failed to get background data. Using fallback...")
            # Create a minimal background structure for demo
            bg = {
                't': np.linspace(0, 200, 1000),
                'a': np.exp(np.linspace(0, 5, 1000)),
                'H': np.ones(1000) * 0.01,
                'phi': np.linspace(-2, 5, 1000),
                'chi': np.zeros(1000),
                'phi_dot': np.ones(1000) * 1e-6,
                'chi_dot': np.zeros(1000),
                'g': np.ones(1000) * 0.5,
                'a_min': 1.73e5,
                't_bounce': 100,
                'N_total': 60.0,
            ...
            print("Using synthetic background for demonstration.")
        
    except ImportError:
        print("Cannot import bounce module. Using synthetic data...")
...
if __name__ == "__main__":
    demonstrate_perturbation_analysis()