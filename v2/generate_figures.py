...
def plot_perturbation_analysis(bg):
    ...
    ax1.semilogy(t, np.abs(turn_rate), 'b-', linewidth=2)
...
def main():
    ...
    print("Running background simulation for numerical figures...")
    bg = run_simulation_robust(phi0=10.0, n_points=100000, t_max=200/omega)
    
    if bg is None:
        print("❌ Background simulation failed! Using fallback data...")
        # For fallback, we'd need to create dummy data, but let's assume success
        return
